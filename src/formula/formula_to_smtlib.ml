...
and visit_bv_term_desc env = function
  ...
  | Select (n,ax,bv) ->
    ...
    let ax = visit_ax_term env ax in
    visit_bind env (ls_ax @ ls_bv)
      (mk_concat (unfold_select env ax bv [] n))
...
and visit_ax_term_desc env = function
  ...
  | Store (n,ax,bv1,bv2) ->
    ...
    let ax = visit_ax_term env ax in
    visit_bind env (ls_ax @ ls_bv1 @ ls_bv2)
...
and unfold_select env ax bv l = function
  ...
  | n -> unfold_select env ax (mk_bv_add_int bv 1)
