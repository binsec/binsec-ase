...
let error msg loc =
  let msg = Format.asprintf "%a:%s" Location.pp_lines loc msg in
  failwith msg
...
let add_int bv n =
  ...
  let bv_n = Bitvector.create (Z.of_int n) sz in
...
let is_int bv vi =
  ...
  | BvCst bv ->
    let sz = Bitvector.size_of bv in
    let v = Bitvector.create (Z.of_int vi) sz in
