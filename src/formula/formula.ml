...
module Printing = struct
  ...
  let bvunop_to_str bv_uop =
    ...
    | BvExtract int ->
      sprintf "extr {%i..%i}" int.Interval.lo int.Interval.hi
  ...
  let rec p_blterm fmt bl =
    ...
    | BlBnop (bl_bop, bl1, bl2) ->
      fprintf fmt "@[<hv2>(%s@ %a@ %a)@]"
        (blbnop_to_str bl_bop) p_blterm bl1 p_blterm bl2
    | BlComp (bl_cmp, bl1, bl2) ->
      fprintf fmt "@[<hv2>(%s@ %a@ %a)@]"
        (blcmp_to_str bl_cmp) p_blterm bl1 p_blterm bl2
    | BvComp (bv_cmp, bv1, bv2) ->
      fprintf fmt "@[<hv2>(%s@ %a@ %a)@]"
        (bvcmp_to_str bv_cmp) p_bvterm bv1 p_bvterm bv2
    | AxComp (ax_cmp, ax1, ax2) ->
      fprintf fmt "@[<hv2>(%s@ %a@ %a)@]"
        (axcmp_to_str ax_cmp) p_axterm ax1 p_axterm ax2
    | BlIte (bl_cond, bl_then, bl_else) ->
      fprintf fmt "@[<hv2>(ite@ %a@ %a@ %a)@]"
        p_blterm bl_cond p_blterm bl_then p_blterm bl_else
  ...
  and p_bvterm fmt bv =
    ...
    | BvBnop (bv_bop, bv1, bv2) ->
      fprintf fmt "@[<hv2>(%s@ %a@ %a)@]"
        (bvbnop_to_str bv_bop) p_bvterm bv1 p_bvterm bv2
    | BvIte (bl_cond, bv_then, bv_else) ->
      fprintf fmt "@[<hv2>(ite@ %a@ %a@ %a)@]"
        p_blterm bl_cond p_bvterm bv_then p_bvterm bv_else
    | Select (n, mem, idx) ->
      fprintf fmt "@[<hv2>(select %i@ %a@ %a)@]"
        n p_axterm mem p_bvterm idx

  and p_axterm fmt ax =
    match ax.ax_term_desc with
    | AxFun ({ ax_name ; _ }, []) ->
      fprintf fmt "%s" ax_name
    ...
    | AxIte (bl_cond, ax_then, ax_else) ->
      fprintf fmt "@[<hv2>(ite@ %a@ %a@ %a)@]"
        p_blterm bl_cond p_axterm ax_then p_axterm ax_else
    | Store (n, mem, idx, elt) ->
      fprintf fmt "@[<hv2>(store %i@ %a@ %a@ %a)@]"
        n p_axterm mem p_bvterm idx p_bvterm elt
      ...
      fprintf fmt "%s@ (@[<hv>%a@])@ %a"
        ...
        (fun fmt ->
           pp_print_list
             ...
             fmt)
        list
        p_sort (AxSort (idx_size, elt_size))
  ...
  and p_define fmt def =
    ...
    | AxDef ({ ax_name ; idx_size ; elt_size ; _ }, [], ax) ->
      fprintf fmt "%s@ %a@ %a"
        ax_name p_sort (AxSort (idx_size, elt_size)) p_axterm ax
    | BlDef ({ bl_name ; _ }, list, bl) ->
      fprintf fmt "%s@ (@[<hv>%a@])@ %a@ %a"
        bl_name
        (fun fmt ->
           pp_print_list
             ~pp_sep:(fun fmt () -> fprintf fmt "@ ")
  ...
  | BvComp (c,bv1,bv2) ->
    ...
    (match is_bv_cst bv1, is_bv_cst bv2 with
     | None, None ->
       ...
       else (* Remove negations (bvnot and bvneg) on both sides of (dis-)equality *)
...
let mk_bl_def v lst bl =
  def (BlDef (v,lst,bl))
let mk_bv_def v lst bv =
  if v.bv_size <> bv.bv_term_size
  then Formula_options.Logger.fatal "defining %s (a %d bit bv var) by %a (a %d bit bv value)"
      v.bv_name v.bv_size
...
let mk_bv_extract      i bv =
  ...
  if not ((i.hi >= i.lo) && (bv.bv_term_size > i.hi) && (i.lo >= 0))
  then Formula_options.Logger.fatal "invalid extract of bits [%i;%i] for bv %a of size %i"
      i.lo
      i.hi
      Printing.p_bvterm bv
      bv.bv_term_size
  ;
...
let mk_bv_add_int bv i =
  mk_bv_add bv
    (mk_bv_cst (Bitvector.create (Z.of_int i) bv.bv_term_size))
