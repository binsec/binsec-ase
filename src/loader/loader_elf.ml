...
let load_file_descr file_descr =
  let buffer = Bigarray.(array1_of_genarray
      (Mmap.V1.map_file file_descr Int8_unsigned C_layout false [|-1|]))
  in load buffer
...
let notes i =
  let cursor =
    Loader_buf.cursor ~at:0 i.Img.header.Ehdr.ident.E_ident.data i.Img.buf in
  Array.fold_left (fun notes s ->
      if s.Shdr.kind = Shdr.SHT.NOTE then begin
        seek cursor s.Shdr.offset;
        Note.append_all notes (s.Shdr.offset + s.Shdr.size) cursor
      end else notes) [] i.Img.sections
