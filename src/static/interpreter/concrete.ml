...
module Image = struct
  ...
  let fetch _ c =
    begin if Caddr.base_value c <> cache.addr then
...
module Goal = struct
  ...
  let echo e ex =
    Logger.result "%a = %a"
      ...
      Bv.pp_hex_or_bin @@ Env.eval e ex

  let step t e a = if a.Dba.id = 0 then
      ...
      List.fold_left
        ...
        if addr = a then
          ...
          | Directive.Enumerate (_, ex) -> echo e ex; action
          | _ -> action
        else action)
        Continue t
    else Continue
