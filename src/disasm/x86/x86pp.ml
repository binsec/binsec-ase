...
let pp_instr ppf instr =
  ...
  | LoadFarPointer (mode, sreg, reg, address) ->
    ...
    fprintf ppf "@[l%a@ %a,@ %a@]"
      pp_segment_reg sreg pp_reg reg pp_address address
  ...
  | Pavgusb (_, _, _, _) -> fprintf ppf "@[pavgusb ...@]"
  ...
  | Stos (rep, `M32)
  | Stos (rep, `M16) -> fprintf ppf "@[%astos@]"  pp_rep rep
  | Stos (rep, `M8) -> fprintf ppf "@[%astosb@]"  pp_rep rep
  | Scas (rep, `M32)
  | Scas (rep, `M16) -> fprintf ppf "@[%ascas@]"  pp_rep rep
...
let pp_bytes nbytes ppf i =
  ...
  let rec aux ppf j =
    if j < nbytes then
      let byte =
        (Z.to_int (Z.extract i (j * bytesize) bytesize)) in
