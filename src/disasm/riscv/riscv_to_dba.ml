...
module Lift = struct
  ...
  let is_ra bv =
    Z.to_int (Bv.value_of bv) = Rar.num Rar.ra
  ;;
  ...
  let load name load_f st ~dst ~src ~offset =
    ...
    let mnemonic =
      Printf.sprintf "%s %s,%s(%s)"
        name (reg_name dst)
        (Z.to_string (Bv.signed_of offset)) (reg_name src) in
    mnemonic, dba
  ;;
  ...
  module C = struct
    ...
    let li st ~dst ~imm5 ~imm1 =
      ...
      Printf.sprintf "li %s,%s"
        (reg_name dst) (Z.to_string (Bv.signed_of imm)),
      dba
    ;;
    ...
    let lui st ~dst ~imm5 ~imm1 =
      assert (let v = Bitvector.value_of dst |> Z.to_int in
              v <> 0 && v <> 2);
    ...
    let apply_shift name lift_f st ~shamt ~dst ~src ~opcode =
      ...
      let mnemonic =
        Printf.sprintf "%s %s,%s,%s"
          ...
          (Z.to_string (Bitvector.value_of shamt)) in
    ...
    let shift name shift_f st opcode =
      ...
      let mnemonic =
        Printf.sprintf "%s %s,%s,%s"
          ...
          (Z.to_string (Bv.value_of shamt)) in
      Inst.create ~dba ~mnemonic ~opcode
    ;;
