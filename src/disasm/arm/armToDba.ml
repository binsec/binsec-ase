...
let get_result =
  ...
  fun ic ->
    try
      rloop ic
    with
    | End_of_file -> close_and_return ic
...
let dummy_parse s =
  ...
  try
    ...
    | i -> Statistics.incr_errors opcode stats; Error (i)
    | exception Failure _ ->
...
let parse_result s =
  ...
  let lexbuf = from_string s in
  try
    Ok (mk_instruction (Parser.decoder_msg Lexer.token lexbuf))
  with _ -> dummy_parse s

let get_and_parse_result ic = get_result ic |> parse_result


let run_external_decoder mode addr bytes =
  let exe = Kernel_options.Decoder.get () in
  let cmd = Format.sprintf "%s %s 0x%x 0x%x" exe mode addr bytes in
  Unix.open_process_in cmd
;;

let decode_from_reader_arm addr reader =
  if addr mod 4 <> 0 then Error (empty_instruction)
  else match Lreader.Peek.u32 reader with
    | exception _ ->
      Statistics.incr_invalid_size stats; Error (empty_instruction)
    | bytes ->
      get_and_parse_result (run_external_decoder "arm" addr bytes)
  ...
  let outer =
    let rec iter i map =
      if i = n then map
        ...
        iter (i + 1)
          (Dba_types.Caddress.Map.add (Dba_types.Caddress.block_start_of_int (addr + 2 * i))
                              (Dba.Jump_target.inner sizes.(i)) map) in
    let map = iter 1 Dba_types.Caddress.Map.empty in
    fun j -> function
      | Dba.JInner goto -> Dba.Jump_target.inner (inner j goto)
      | Dba.JOuter caddr as jo ->
        try Dba_types.Caddress.Map.find caddr map with Not_found -> jo in
  let open Dhunk in
  let j = ref 0 in
  init sizes.(n)
    (fun i ->
  ...
  for i = 0 to Bitvector.size_of bv / 8 - 1 do
    Format.fprintf ppf "%02x"
      (Z.to_int
         Bitvector.(value_of
                      (extract bv
                         { Interval.lo = 8 * i; hi = 8 * (i + 1) - 1 })))
  done

let itstate w i =
  if i = 0 then 0
  else (w land 0xe0) lor ((w lsl (i - 1)) land 0x1f)

let pp_itstate =
  let rec iter i ppf t =
    if i < 8 then begin
      if t land (0x1 lsl (7 - i)) <> 0 then
        Format.pp_print_char ppf '1'
      else
        Format.pp_print_char ppf '0';
      iter (i + 1) ppf t
    end in
  iter 0

let modeit w = Format.asprintf "thumb,itstate=%a" pp_itstate w

let isitw w = let w = w land 0xffff in 0xbf00 < w && w <= 0xbfff

let decode_from_reader_thumb addr reader =
  let addr = if addr mod 2 = 1 then
      begin Lreader.rewind reader 1; addr - 1 end
    else addr in
  match try Lreader.Peek.u32 reader with _ -> Lreader.Peek.u16 reader with
  | exception _ -> Statistics.incr_invalid_size stats; Error (empty_instruction)
  | bytes when not @@ isitw bytes ->
    get_and_parse_result (run_external_decoder "thumb,outitb" addr bytes)
  | word -> (* it block *)
    let n = 1 +  if word land 0x01 <> 0 then 4
    ...
    let rec init i offset = if i = n then offset
      else match try Lreader.Peek.peek reader 4
          with _ -> Lreader.Peek.peek reader 2 with
      | exception _ -> raise @@ Failure ""
      | bytes ->
        match get_and_parse_result
                (run_external_decoder (modeit (itstate word i))
                   (addr + offset) (Bitvector.to_int bytes))
        with
        | Error _ -> raise @@ Failure ""
        | Ok (instr, dhunk) ->
          itblock.(i) <- instr; ithunks.(i) <- dhunk;
          let size = Size.Byte.to_int instr.Instruction.Generic.size in
          Lreader.advance reader size;
          init (i + 1) (offset + size) in
    try
      ...
      let mnemonic = Mnemonic.supported itblock
          (fun ppf it -> Array.iteri
              (fun i g ->
                 if i <> 0 then Format.pp_print_string ppf "; ";
                 Instruction.Generic.pp_mnemonic ppf g) it) in
      Ok (Instruction.Generic.create size opcode mnemonic,
          merge_itblock addr ithunks)
    with Failure _ -> Error (empty_instruction)

let thumb_mode = Dba.(Expr.var ~tag:(VarTag.flag Flag.unspecified) "t" 1)
...
let assert_mode mode dhunk =
  ...
  init (length dhunk + 1)
    (function
      ...
      | i -> Dba_types.Instruction.reloc ~inner:(fun x -> x + 1)
...
let merge_dhunk arm thumb =
  ...
  init (1 + larm + lthumb)
    (function
      ...
      | i when i <= larm ->
        Dba_types.Instruction.reloc ~inner:(fun x -> x + 1)
          (Utils.unsafe_get_opt (inst arm (i - 1)))
      | i ->
        Dba_types.Instruction.reloc ~inner:(fun x -> x + larm + 1)
...
let merge_ginstr arm thumb =
  ...
  let size, opcode =
    ...
    else thumb.size, thumb.opcode in
  let mnemonic = match arm.mnemonic, thumb.mnemonic with
  | Mnemonic.Supported s, Mnemonic.Supported s' ->
    Mnemonic.supported ()
      (fun ppf _ -> Format.fprintf ppf "[arm] %s | [thumb] %s" s s')
  | _, _ -> assert false in
  create (Size.Byte.to_int size) opcode mnemonic


let merge_result arm thumb = match arm, thumb with
  | Error (_), Error (i) -> i
  | Error (_), Ok (g, d) ->
    Statistics.incr_decoded (g, d) stats; g, assert_mode thumb_mode d
  | Ok (g, d), Error (_) ->
...
let decode_from_reader addr reader =
  ...
  | Arm_options.Arm ->
    decode_from_reader_arm addr reader |> unwrap_result
  | Arm_options.Thumb ->
    decode_from_reader_thumb addr reader |> unwrap_result
