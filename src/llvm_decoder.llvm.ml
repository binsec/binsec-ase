...
module Binary = struct
  ...
  let biconst ~size i llst =
    ...
    (L.const_of_int64 (L.integer_type llst.llcontext size)
       (Transfer_functions.Integer.to_int64 i) false, llst)
...
let decode_dba block =
  ...
  Dhunk.iteri ~f:(fun id inst ->
      ...
      | JKAssume(cond,target) ->
        let llblock = get_jump_target target in
        let _ = L.build_cond_br cond llblock (Lazy.force unreachable)
            llbuilder in
        ()
    ) block;
...
let check_hex_string s =
  ...
  | Some i ->
    begin
      Logger.fatal
        "Invalid hexadecimal character '%c' in opcode %s" s.[i] s
    end
  | None -> ()
...
let decode_llvm raw =
  try
    ...
    Logger.result "%a" pretty i.Instruction.dba_block
  with
...
let run_decode_llvm () =
  if Decode.is_set () then
    decode_llvm (Decode.get ())
...
let run input_file =
  if Output.is_set() then
    run_llvm input_file @@ Output.get()
...
let _ =
  Cli.Boot.enlist ~name:"xtrasec llvm run" ~f:main
;;
