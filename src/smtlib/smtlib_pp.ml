...
let rec pp_term_k k ppf term =
  ...
  | TermSpecConstant sc -> pp_spec_constant ppf sc; k ()
  | TermQualIdentifier qualid -> pp_qual_identifier ppf qualid; k ()
  | TermQualIdentifierTerms (qualid, terms) ->
    fprintf ppf "@[<hov 0>(%a@ " pp_qual_identifier qualid;
    pp_terms_k (fun () -> fprintf ppf ")@]"; k ()) ppf terms
  | TermLetTerm (varbindings, term) ->
    fprintf ppf "@[<hov 0>(let@ (@[<v 0>";
    pp_var_bindings_k
      (fun () ->
         fprintf ppf "@])@ ";
         pp_term_k (fun () -> fprintf ppf ")@]"; k ()) ppf term)
      ppf varbindings
  ...
  | TermAnnotatedTerm (term, attrs) ->
    fprintf ppf "@[<hov 1>(!@ ";
    pp_term_k
      (fun () -> fprintf ppf " %a)@]" pp_attributes attrs; k ())
      ppf term
  | TermLambdaTerm (svars, term) ->
    fprintf ppf "@[<hov 1>(lambda@ (@[<hov>%a@])@ " pp_sorted_vars svars;
    pp_term_k (fun () -> fprintf ppf ")@]"; k ()) ppf term

and pp_terms_k k ppf terms =
  pp_list_k pp_term_k k ppf terms
