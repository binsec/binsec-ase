...
let pp_goal ppf = function
  | Reach (c, Dba.Expr.Cst _) -> fprintf ppf "reach %a" Count.pp c
  | Reach (c, e) -> fprintf ppf "reach %a such as %a = true" Count.pp c
                      Dba_printer.Ascii.pp_bl_term e
  | Enumerate (c, _e) ->
     fprintf ppf "enum %d" c
...
let reach ?(n=1) ?(guard=Dba.Expr.one) ~loc () =
  assert (n >= 0);
  assert (not (Dba.Expr.is_equal guard Dba.Expr.zero));
  { loc; goal = Reach (Count.count n, guard); }

let reach_all ~loc () = { loc; goal = Reach (Count.unlimited, Dba.Expr.one); }
...
let check_and_decr g =
  ...
  | Assume _
  | Cut _
  | Reach ((Count.Unlimited | Count.Count 0), _)
  | Enumerate (0, _) -> None
  | Reach ((Count.Count n as c), cond) ->
     assert (n >= 1);
     Some { g with goal = Reach (Count.decr c, cond); }
  | Enumerate (n, e) ->
     assert (n >= 1);
