...
let vertex_list g =
  ...
  |> List.sort (fun x y -> compare (C.V.addr x) (C.V.addr y))
...
module Check = struct
  ...
  module N =
    ...
    struct
      ...
      let equal (r, s) (r', s') = r = r' && Basic_types.String.Set.equal s s'
      let analyze e (r, s) =
        let src = C.E.src e in
        match C.V.inst src with
        | None -> assert false
        (* there should not be a vertex without an instruction *)
        | Some i ->
          r, Basic_types.String.Set.union (DI.temporaries i).Dba_types.defs s
    end
    )

  let no_temporary_leak g =
    let module Strg = Basic_types.String in
    let start = start g in
    let init v =
      match C.V.inst v with
      | None -> assert false (* same here, none is ill formed *)
      | Some i -> v = start, (DI.temporaries i).Dba_types.defs
    in
