...
let section img ?(at=0) name =
  let sec =
    ...
    with Not_found -> Dwarf_options.Logger.fatal "missing section %s" name in
      ...
      if cursor.position mod 16 = 0 then
        Format.fprintf ppf "@ 0x%a"
          (Machine.Bitwidth.pp_print_hex
             (Kernel_options.Machine.bits ())) cursor.position;
      if cursor.position mod 4 = 0 then
        Format.pp_print_char ppf ' ';
      Format.fprintf ppf "%02x" (Read.u8 cursor);
      if cursor.position mod 16 = 0 then begin
          Format.pp_print_char ppf ' ';
          advance cursor (-16);
          for _ = 0 to 15 do
            Format.pp_print_char ppf
...
module Tag = struct
  ...
  let pp ppf = function
    ...
    | Imported_declaration     -> Format.fprintf ppf
    ...
    | Unspecified_parameters   -> Format.fprintf ppf
    ...
    | Template_type_parameter  -> Format.fprintf ppf
                                    "DW_TAG_template_type_parameter"
    | Template_value_parameter -> Format.fprintf ppf
    ...
    | Rvalue_reference_type    -> Format.fprintf ppf
    ...
    | Call_site_parameter      -> Format.fprintf ppf
...
let of_value = function
  ...
  module MapSet = Basic_types.Collection_make.Default
      (struct type a = t type t = a let compare = compare end)
  ...
  let pp ppf = function
    ...
    | String_length_bit_size  -> Format.fprintf ppf
                                   "DW_AT_string_length_bit_size"
    | String_length_byte_size -> Format.fprintf ppf
    ...
    | Call_all_source_calls   -> Format.fprintf ppf
    ...
    | Call_target_clobbered   -> Format.fprintf ppf
...
module rec Class : sig
  ...
  let pp ppf = function
    ...
    | Reference r   ->
      let attrs = match r () with
          | Die.Leaf (_, attrs) -> attrs
          | Die.Node (_, attrs, _) -> attrs in
      if Attribute.Map.mem Attribute.Name attrs then
        Format.fprintf ppf "Reference to %a"
          Class.pp (Attribute.Map.find Attribute.Name attrs)
...
and Die : sig
  ...
  let get attr = function
    | Leaf (_, attrs)
    | Node (_, attrs, _) -> Attribute.Map.find attr attrs

  let rec pp ppf die : unit =
    let common tag attributes = Format.fprintf ppf "%a@ %a" Tag.pp tag
        (fun ppf attrs -> Attribute.Map.iter
            (fun attr klass -> Format.fprintf ppf "%a : %a@ "
                Attribute.pp attr Class.pp klass) attrs) attributes in
    match die with
    | Leaf (tag, attributes) -> common tag attributes
    | Node (tag, attributes, childrens) -> common tag attributes;
      Format.fprintf ppf "@ @ ";
      let rec loop = function
        | [] -> ()
        | children :: tail -> pp ppf children; Format.fprintf ppf "@ ";
          loop tail in
      loop childrens
end
...
module Header = struct
  ...
  let pp ppf h : unit =
    Format.fprintf ppf "@[<v>Length: 0x%x (%a-bit)@ " h.length
      Machine.Bitwidth.pp (h.format :> Machine.bitwidth);
...
module Form = struct
  ...
  let load cursor header strings dies = function
    ...
    | Block2 -> Class.Block
                  (String.init (Read.u16 cursor)
                     (fun _ -> Char.chr (Read.u8 cursor)))
    | Block4 -> Class.Block
                  (String.init (Read.u32 cursor)
                     (fun _ -> Char.chr (Read.u8 cursor)))
    ...
    | Block  -> Class.Block
                  (String.init (Read.uleb128 cursor)
                     (fun _ -> Char.chr (Read.u8 cursor)))
    | Block1 -> Class.Block
                  (String.init (Read.u8 cursor)
                     (fun _ -> Char.chr (Read.u8 cursor)))
    ...
    | Ref_addr -> ignore @@ Header.read_addr header cursor;
...
module Abbrev = struct
    ...
    let read cursor : (int * t) option =
      ...
      | no ->
        ...
        let rec loop encodings =
          ...
          | None-> advance cursor 1; List.rev encodings
          | Some encoding -> loop (encoding :: encodings) in
        let encodings = loop [] in
        Some (no, (tag, has_children, encodings))

    let pp ppf (tag, has_children, encodings) : unit =
      Format.fprintf ppf "@[<v 1>%a [%a]@ %a@]"
        Tag.pp tag
        (fun ppf -> function
           | true -> Format.fprintf ppf "has children"
           | false -> Format.fprintf ppf "no children") has_children
        (fun ppf encodings ->
           let rec loop = function
             | [] -> ()
             | [encoding] -> Format.fprintf ppf "%a" Encoding.pp encoding
             | encoding :: tail -> Format.fprintf ppf "%a@ "
                                     Encoding.pp encoding;
               loop tail in
           loop encodings) encodings
  end
  ...
  let load cursor : t =
    let rec loop i abbrevs =
      ...
      | Some (no, abbrev) -> assert (i = no);
        loop (i + 1) (abbrev :: abbrevs) in
    let abbrevs = loop 1 [] in
    abbrevs

  let pp ppf abbrevs : unit = Array.iteri
      (fun no abbrev ->
         Format.fprintf ppf "%4d %a@ " (no + 1) A.pp abbrev) abbrevs
end
...
let load img : t list =
  ...
  let rec outer units =
    if at_end cursor then List.rev units else
      ...
      let rec inner attrs = function
        ...
        | (attr, form) :: tail ->
           ...
           with Failure msg ->
             Dwarf_options.Logger.warning "%s" msg; inner attrs tail in
      let rec load sibling =
        ...
        | x ->
           ...
           let die = match has_children with
             | false -> Die.Leaf (tag, attributes)
             | true  -> Die.Node (tag, attributes, load []) in
...
module Var = struct
  ...
  let rec walk = function
    | [] -> raise Not_found
    | t :: l -> match t with
      | Die.Leaf (Tag.Variable, _)
      | Die.Leaf (Tag.Formal_parameter, _) ->
        begin match name t, line t with
          | name, line -> if name = vname && line = vline then t else walk l
          | exception Not_found -> walk l
        end
    | Die.Leaf _ -> walk l
    | Die.Node (_, _, subtrees) -> List.concat [subtrees; l] |> walk in
  walk [unit]
...
module Func = struct
  ...
  let cfa t = Class.Dwarf_exprloc.get
      (Die.get Attribute.Dwarf_frame_base t)
  ...
  let vars = function
    ...
    | Die.Node (_, _, subtrees) ->
      let rec walk vars = function
        ...
        | d :: l -> match d with
          | Die.Leaf (Tag.Variable, _)
          | Die.Leaf (Tag.Formal_parameter, _) ->  walk (d :: vars) l
          | Die.Node (_, _, subtrees) ->
  ...
  let vars = function
    ...
    | Die.Node (_, _, subtrees) ->
      let rec walk vars = function
        ...
        | d :: l -> match d with
          | Die.Leaf (Tag.Subprogram, _)
          | Die.Node (Tag.Subprogram, _, _) -> walk vars l
