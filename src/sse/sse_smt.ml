      ...
      let keep_addr (addr:Bitvector.t) =
        ...
        *)
        not (Path_state.address_belongs_to_init ~addr path_state) &&
          let address = Bitvector.value_of addr |> Z.to_int in
...
module Translate = struct
  ...
  let nondet ?naming_hint ?(wild=false) lvalue path_state =
    ...
    match lvalue, naming_hint with
    | Dba.LValue.Restrict _, _ | Dba.LValue.Store _, _ | _, Some _ ->
    ...
    | Dba.LValue.Var { name; _ }, None ->
      let symstate =
        Sse_symbolic.State.havoc ~wild name
          (Formula.BvSort size) symstate in
      Path_state.set_symbolic_state symstate path_state
  ...
  let assume cond state =
    ...
    let tr_cond =
      Formula.(mk_bv_equal c mk_bv_one)
        in Path_state.set_symbolic_state st state |>
           Path_state.add_assertion tr_cond
  ;;
