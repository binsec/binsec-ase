...
let sugar :=
  ...
  | v=INT; ~=binop; ~=sugar;
    { let size = Dba.Expr.size_of sugar in
      let value = Dba.Expr.constant @@ Bitvector.of_int ~size v in
  ...
  | hd=isugar; tl=nonempty_list(pair(cmpop, isugar));
	...
	(fun (e, a) (op, b) ->
	  let x, y = match a, b with
	    ...
	    | Left x, Right y ->
	       Dba.Expr.constant
	       @@ Bitvector.of_int ~size:(Dba.Expr.size_of y) x, y
	    | Right x, Left y ->
	       x, Dba.Expr.constant
		  @@ Bitvector.of_int ~size:(Dba.Expr.size_of x) y
...
let ite :=
  | test=bool; QMARK; branch=ilogic; COLON; default=iexpr;
	...
	| Left x,  Right y ->
	   Dba.Expr.constant (Bitvector.of_int ~size:(Dba.Expr.size_of y) x), y
	| Right x, Left y  ->
	   x, Dba.Expr.constant (Bitvector.of_int ~size:(Dba.Expr.size_of x) y)
	| Right x, Right y -> x, y in
  ...
  | CASE; expr=logic; IS;
	...
	| Left x ->
	   let size' =
	     ...
	     (-1) cases in
	   assert (size' <> -1); (* TODO *)
	   Dba.Expr.constant (Bitvector.of_int ~size:size' x), size'
	| Right y -> y, Dba.Expr.size_of y in
      List.fold_left
	(fun e (k, c) ->
	  let k = match k with
	    | Left x -> Dba.Expr.constant (Bitvector.of_int ~size x)
	    | Right y -> Dba.Expr.constant y in
	  let c = match c with
	    | Left x -> Dba.Expr.constant (Bitvector.of_int ~size:size' x)
	    | Right y -> y in
...
let addr :=
  ...
  | value=INT;
    { Dba.Expr.constant @@ Bitvector.of_int ~size:E.wordsize value }
