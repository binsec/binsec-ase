...
= struct
  ...
  let _is_zero = function
    | Cst bv -> Bitvector.is_zeros bv
    | _ -> false

  let _is_one = function
    | Cst bv -> Bitvector.is_ones bv
    | _ -> false

  let is_max = function
    | Cst bv -> Bitvector.is_max_ubv bv
    | _ -> false
  ...
  let ite condition then_expr else_expr =
    ...
    | Cst b when Bitvector.is_zero b ->
      else_expr
    | Cst b when Bitvector.is_one b ->
      then_expr
    | _ ->
  ...
  *)
  ...
  and uext size = function
    ...
    | Cst bv ->
      constant Bitvector.(extend bv size)
    | Unary (Unary_op.Uext _, e) | e -> Straight.uext size e

  and sext size = function
    | e when size = size_of e -> e
    | Cst bv ->
      constant Bitvector.(extend_signed bv size)
  ...
  and restrict lo hi = function
    ...
    | Cst bv ->
      constant (Bitvector.extract bv Interval.{lo; hi})
    ...
    | Unary (Unary_op.Uext _, e) when size_of e <= lo ->
      zeros (hi - lo + 1)
    ...
    | Binary (Binary_op.LShift, _, Cst b2)
      when Bitvector.to_uint b2 > hi ->
      zeros (hi - lo + 1)
    | Binary (Binary_op.LShift, e1, Cst b2)
      when Bitvector.to_uint b2 <= lo ->
  ...
  and add e1 e2 = match e1, e2 with
    ...
    | Binary (Binary_op.Plus, e3, (Cst _ as c4)), _ ->
      add (add e3 e2) c4
    | Binary (Binary_op.Minus, e3, Cst b1), Cst b2
      when Bitvector.sge b2 b1 ->
  ...
  and sub e1 e2 = match e1, e2 with
    ...
    | Cst b1, Binary (Binary_op.Minus, Cst b2, e3)
      when Bitvector.sge b1 b2 -> add e3 (constant (Bitvector.sub b1 b2))
    | Cst b1, Binary (Binary_op.Minus, Cst b2, e3) ->
    ...
    | Binary (Binary_op.Minus, e3, (Cst _ as c4)), _ ->
      sub (sub e3 e2) c4
    (* Neutral element *)
  ...
  and umod e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.umod b1 b2)
    | _, _ -> Straight.umod e1 e2

  and sdiv e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.sdiv b1 b2)
    (* neutral element *)
    | _, Cst b2 when Bitvector.is_ones b2 -> e1
  ...
  and smod e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.smod b1 b2)
    | _, _ -> Straight.smod e1 e2

  and logxor e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.logxor b1 b2)
    ...
    | Cst b1, Binary (Binary_op.Concat, e3, e4) ->
      ...
      let b3 = Bitvector.extract b1 {Interval.lo = s4; Interval.hi = s2 - 1} in
      let b4 = Bitvector.extract b1 {Interval.lo = 0; Interval.hi = s4 - 1} in
  ...
  and logor e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.logor b1 b2)
    | Cst b1, _ when Bitvector.is_zeros b1 -> e2
    ...
    | _,
      Binary (Binary_op.LShift, Unary (Unary_op.Uext _, e3), Cst b1) ->
      let at = Bitvector.to_uint b1 in
      try_merge ~k:(fun _ -> Straight.logor e1 e2) e3 ~at e1
    | Unary (Unary_op.Uext _, _), _
    | Binary (Binary_op.LShift, Unary (Unary_op.Uext _, _), Cst _),
      _ -> logor e2 e1
  ...
  and logand e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.logand b1 b2)
    | Cst b1, _ when Bitvector.is_zeros b1 -> e1
  ...
  and append e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.append b1 b2)
    | Cst b1, _ when Bitvector.is_zeros b1 ->
      uext (size_of e1 + size_of e2) e2
    | Unary (Unary_op.Uext s1, e3), _ ->
      uext (s1 + size_of e2) (append e3 e2)
    ...
    | Binary (Binary_op.Concat, e3, e4), _ ->
      append e3 (append e4 e2)
    | _, _ -> Straight.append e1 e2

  and equal e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.equal b1 b2))
  ...
  and diff e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.diff b1 b2))
    | Cst _, Binary (Binary_op.Concat, e3, e4) ->
  ...
  and ult e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.ult b1 b2))
    | _, _ when is_equal e1 e2 -> zero
    | _, _ -> Straight.ult e1 e2

  and ule e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.ule b1 b2))
    | _, _ when is_equal e1 e2 -> one
    | _, _ -> Straight.ule e1 e2

  and ugt e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.ugt b1 b2))
    | _, _ when is_equal e1 e2 -> zero
    | _, _ -> Straight.ugt e1 e2

  and uge e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.uge b1 b2))
    | _, _ when is_equal e1 e2 -> one
    | _, _ -> Straight.uge e1 e2

  and slt e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.slt b1 b2))
    | _, _ when is_equal e1 e2 -> zero
    | _, _ -> Straight.slt e1 e2

  and sle e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.sle b1 b2))
    | _, _ when is_equal e1 e2 -> one
    | _, _ -> Straight.sle e1 e2

  and sgt e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.sgt b1 b2))
    | _, _ when is_equal e1 e2 -> zero
    | _, _ -> Straight.sgt e1 e2

  and sge e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.of_bool (Bitvector.sge b1 b2))
    | _, _ when is_equal e1 e2 -> one
  ...
  and shift_left e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.shift_left b1 (Bitvector.to_uint b2))
      (* w << 0 *)
    | _, Cst b2 when Bitvector.is_zeros b2 -> e1
      (* 0 << w *)
    | Cst b1, _ when Bitvector.is_zeros b1 -> e1
  ...
  and shift_right e1 e2 = match e1, e2 with
    | Cst b1, Cst b2 ->
      constant (Bitvector.shift_right b1 (Bitvector.to_uint b2))
      (* w >> 0 *)
    | _, Cst b2 when Bitvector.is_zeros b2 -> e1
      (* 0 >> w *)
    | Cst b1, _ when Bitvector.is_zeros b1 -> e1
  ...
  and try_merge ~k e1 ~at e2 =
    ...
    if at + sz1 <= sz2 && restrict at (at + sz1 - 1) e2 = zeros sz1 then
      if at = 0 then
        append (restrict sz1 (sz2 - 1) e2) e1
      else if at + sz1 = sz2 then
        append e1 (restrict 0 (at - 1) e2)
      else
        append
...
module Var = struct
  ...
  let flag ?(bitsize=Size.Bit.bits1) ?(flag_t=Flag.unspecified) flagname =
    create flagname ~bitsize ~tag:(VarTag.flag flag_t)
...
(*    Format.printf "store : %d@." (Expr.size_of e); *)
  ...
  let bitsize = function
    ...
    | Store (sz, _endianness, _e) ->
      Size.Byte.(to_bitsize (create sz))
...
module Instr = struct
  ...
  let ite c goto nid =
    if Expr.(is_equal c zero) then
      static_inner_jump nid
    else if Expr.(is_equal c one) then
      static_jump goto
    else If (c, goto, nid)
