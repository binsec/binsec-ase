...
module Expr = struct
  ...
  let of_vaddr (v : Virtual_address.t) = Expr.constant
  ...
  let eval_from_img =
    ...
    | Dba.Expr.Var { name; size; _ } ->
      Dba.Expr.constant @@ Bitvector.of_int ~size @@
      Option.get @@ Loader_utils.address_of_symbol_by_name img ~name
    ...
    | Dba.Expr.Ite (c, t, e) ->
      Dba.Expr.ite (fold img c) (fold img t) (fold img e) in
    fun img e ->
  ...
  let complement e ~lo ~hi var =
    let size = var.size and evar = Dba.Expr.v var in
      if lo = 0 then
        Dba.Expr.append
          (Dba.Expr.restrict (hi + 1) (size - 1) evar)
          e
