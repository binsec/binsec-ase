...
let create value size =
  ...
  if size < Sys.int_size - 10 then
    unsafe_of_unboxed (unsafe_to_int (Z.logand value
                                        (Z.pred (Z.shift_left Z.one size)))
                       lsl 10 lor size)
  else if size < 1024 && is_small_int value &&
     (unsafe_to_int value lsl 10) asr 10 = unsafe_to_int value then
    unsafe_of_unboxed (unsafe_to_int value lsl 10 lor size)
  else
    ...
    let signed =
      if Z.lt unsigned slimit then unsigned
      else Z.sub unsigned ulimit in
    unsafe_of_boxed { size; unsigned; signed }

let size_of t =
  if is_unboxed t then
    unsafe_to_int (unsafe_to_unboxed t) land 0x3ff
  else
    let { size; _ } = unsafe_to_boxed t in size
...
let pow bv1 bv2 = unsigned_apply (fun z1 z2 -> Z.pow z1 (Z.to_int z2))
    bv1 bv2 "pow"
...
let rotate_left bv i =
  ...
  update bv
    (Z.logor
       (Z.shift_left value i)
       (Z.shift_right value (size_of bv - i)))

let rotate_right bv i =
  let value = value_of bv in
  update bv
    (Z.logor
       (Z.shift_right value i)
       (Z.shift_left value (size_of bv - i)))
...
let set_bit bv i =
  update bv (Z.logor (bit_mask i) (value_of bv))

let clear_bit bv i =
  update bv (Z.logand (bit_mask_not i) (value_of bv))
...
let to_bool x =
  if size_of x <> 1 then raise Z.Overflow
  else Z.equal (value_of x) Z.zero
...
let pp_hex ppf bv =
  Format.fprintf ppf "{%s; %i}" (to_hexstring bv) (size_of bv)
;;
