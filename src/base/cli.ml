...
let after_parsing v = if (!done_parsing)
  then v
  else
  ...
  module Builder = struct
    ...
    module Integer_option(P : sig include CLI_DECL end) =
      struct
        ...
        let get () =
          assert(is_set ());
          after_parsing @@ match !value with
          | None -> assert false
          | Some v -> v
      ...
      struct
        ...
        and spec =
          (* Scan (fun s ->
            ...
            Scan (fun s ->
              if List.mem s choices then set s
              else scan_error s values
            )
  ...
  let run_on ?(origin=Command_line) cli_options =
    ...
    let rec arg_loop ~last_action i =
      if i < len then
        ...
        L.info ~level "Options %s" switch;
        try
          match H.find args switch with
          | {spec = Unit f; _} as action ->
             L.info ~level "Setting %s" switch;
             f ();
          ...
          | {spec = Scan f; doc; _} as action ->
             ...
             (try
               L.info ~level "Setting %s with %s" switch arg;
               f arg;
             with
             | e ->
               L.fatal ~e:(Cli_parse_error ("Error parsing option " ^ switch))
                 "@[<v 5>Error parsing %s %s@.%s@.%s@]"
                 switch arg (Printexc.to_string e) doc
             );
                 ...
                 | Config_file ->
                    ...
                    ( match last_action with
                      | Some {spec = Unit _; key; _ } ->
                         let msg =
                           Printf.sprintf
                             "Error parsing switch %s. \
                              Unexpected value %s after '=' \
                      ...
                      | None  ->
                         L.fatal
                           ~e:(Cli_parse_error ("Unkown error at value " ^ switch))
                           "Got value %s. Don't know what to do."
                           switch)
                 );
  ...
  let split_on_sep line =
    ...
    match index line arg_sep with
      | i ->
         ...
         trim @@ sub line 0 i,
         Some (trim @@ sub line (i + 1) (len - 1 - i))
