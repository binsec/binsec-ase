...
let  mk_assert st e =
  ...
  let open Formula in
  let bv_term, st = Sse_smt.Translate.expr st e in
  let bv_true = mk_bv_cst Bitvector.one in
  (mk_bv_comp BvEqual bv_term bv_true, st)
...
module Translator = struct
  ...
  let add_assert_constraint t e =
    let assertion, state = mk_assert t.state e in
    let state = Sse_symbolic.State.constrain assertion state
    in { t with state }

  let  add_deny_constraint t e = add_assert_constraint t (Dba.Expr.lognot e)
  ...
  module Eval = struct
    ...
    let ite e jt _next_id t =
      ...
      | Dba.JOuter addr ->
         begin
           let t = if next_is_addr addr t then add_assert_constraint t e
           else add_deny_constraint t e in
           step t
         end
  end
  ...
  let run t =
    ...
    and do_dba t dba =
      ...
      | DJump (e, Some Dba.Return) ->
         ...
         let bv_term, state = Sse_smt.Translate.expr t.state e in
...
let init_mem ~filename symb_st =
  if not (Sys.file_exists filename) then symb_st
    ...
    let f syst init =
      ...
      | Assumption e ->
        let msg =
           Format.asprintf "Unhandled assumption %a"
             Dba_printer.EICAscii.pp_bl_term e in
         failwith msg
...
let check_trace c tr =
  ...
  | t, Some _ ->
     ...
     let c_sat = solve ~title:"consequent"  Translator.(add_assert_constraint t e)
     and a_sat = solve ~title:"alternative" Translator.(add_deny_constraint t e) in
